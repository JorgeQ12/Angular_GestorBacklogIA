...
La consulta vive en `/panel/proyectos`. Es el destino de la navegación principal y de los accesos
desde Inicio. La creación permanece en `/panel/proyectos/creacion`; abrir la consulta no monta el
...
│   ├── models/
│   ├── pages/
│   │   └── pagina-consulta-proyectos/
│   └── services/
├── creacion/
└── secciones/
```

- `PaginaConsultaProyectos` compone filtros, resultados y navegación.
- `FormularioFiltrosProyectos` administra un formulario reactivo tipado, aplica una espera
  breve a la búsqueda y emite una fotografía completa de los criterios.
- `filtros-proyectos.mapper` centraliza la normalización y comparación de filtros para que
...
- `ConsultaProyectosService` ejecuta `ObtenerProyectos`, valida `ResultadoApi` y adapta el DTO.
- `EstadoConsultaProyectosService` conserva únicamente el estado de la ruta, cancela consultas
  anteriores y diferencia error, vacío y datos disponibles.

El estado se proporciona en la ruta base de la consulta y no como un facade global. Así no se filtran
...
la consulta y el modelo presentado permanecen dentro de `proyectos/consulta`.
...
resultados porque la consulta ya comunica su contenido y paginación.
...
- La consulta no incluye edición en modal. Actualizar datos generales y crear una versión pertenece
...
`estado`; la consulta valida el valor y descarta cualquier parámetro que no pertenezca al enum. La
...
configuraciones ni servicios internos de la consulta.
