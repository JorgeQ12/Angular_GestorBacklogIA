...
describe('avancePasoCreacionProyectoGuard', () => {
  ...
  it('permite abrir un paso ya alcanzado', async () => {
    ...
    await expect(ejecutarGuard('necesidad')).resolves.toBe(true);
  });
  ...
  it('redirige al último paso disponible cuando se intenta saltar el recorrido', async () => {
    ...
    await expect(ejecutarGuard('necesidad')).resolves.toBe(
  ...
  it('redirige a Alcance cuando Objetivos ya fue completado', async () => {
    ...
    await expect(ejecutarGuard('roles')).resolves.toBe('/panel/proyectos/42/creacion/alcance');
  });
  ...
  it('permite abrir Roles cuando Alcance ya fue completado', async () => {
    ...
    await expect(ejecutarGuard('roles')).resolves.toBe(true);
  });
  ...
  function ejecutarGuard(pasoActual: string): Promise<GuardResult> {
    const ruta = {
      data: { pasoActual },
