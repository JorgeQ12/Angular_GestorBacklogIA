...
import type { CambioPaginaUsuarios, Usuario } from '../../models/usuario.model';
...
export class TablaUsuarios {
  ...
  /** Proporciona la página que se encuentra representada. */
  ...
  /** Solicita otra página sin asumir cómo se recupera. */
  public readonly paginaCambiada = output<CambioPaginaUsuarios>();
  ...
  /** Solicita la página anterior cuando existe. */
  protected anterior(): void {
    if (!this.bloqueado() && this.paginaActual() > 1) {
      this.paginaCambiada.emit({ pagina: this.paginaActual() - 1 });
    }
  }

  /** Solicita la página siguiente cuando existe. */
  protected siguiente(): void {
    if (!this.bloqueado() && this.paginaActual() < this.totalPaginas()) {
      this.paginaCambiada.emit({ pagina: this.paginaActual() + 1 });
    }
  }
}
